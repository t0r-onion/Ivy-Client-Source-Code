...
public class Stitcher {
    ...
    public void doStitch() {
        ...
        for (int i = 0; i < n; ++i) {
            Object stitcher$holder = arrobject[i];
            if (this.allocateSlot((Holder)stitcher$holder)) continue;
            String s = String.format("Unable to fit: %s - size: %dx%d - Maybe try a lowerresolution resourcepack?", ((Holder)stitcher$holder).getAtlasSprite().getIconName(), ((Holder)stitcher$holder).getAtlasSprite().getIconWidth(), ((Holder)stitcher$holder).getAtlasSprite().getIconHeight());
