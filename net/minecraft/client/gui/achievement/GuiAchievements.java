...
implements IProgressMeter {
    ...
    protected void drawAchievementScreen(int p_146552_1_, int p_146552_2_, float p_146552_3_) {
        ...
        for (int j5 = 0; j5 < AchievementList.achievementList.size(); ++j5) {
            ...
            if (k5 > j6) {
                GuiAchievements.drawTexturedModalRect(k5 - 11 - 7, l5 - 5, 114, 234, 7, 11);
                continue;
            }
            if (k5 < j6) {
                GuiAchievements.drawTexturedModalRect(k5 + 11, l5 - 5, 107, 234, 7, 11);
                continue;
            }
            if (l5 > k6) {
                GuiAchievements.drawTexturedModalRect(k5 - 5, l5 - 11 - 7, 96, 234, 11, 7);
                continue;
            }
            if (l5 >= k6) continue;
            GuiAchievements.drawTexturedModalRect(k5 - 5, l5 + 11, 96, 241, 11, 7);
        }
        ...
        for (int i6 = 0; i6 < AchievementList.achievementList.size(); ++i6) {
            ...
            if (achievement2.getSpecial()) {
                GuiAchievements.drawTexturedModalRect(l6 - 2, j7 - 2, 26, 202, 26, 26);
            } else {
                GuiAchievements.drawTexturedModalRect(l6 - 2, j7 - 2, 0, 202, 26, 26);
            }
        ...
        GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
        this.mc.getTextureManager().bindTexture(ACHIEVEMENT_BACKGROUND);
        GuiAchievements.drawTexturedModalRect(k, l, 0, 0, this.field_146555_f, this.field_146557_g);
        zLevel = 0.0f;
        GlStateManager.depthFunc(515);
        GlStateManager.disableDepth();
        GlStateManager.enableTexture2D();
