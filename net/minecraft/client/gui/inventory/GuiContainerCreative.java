...
extends InventoryEffectRenderer {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if (mouseButton == 0) {
            ...
            CreativeTabs[] arrcreativeTabs = CreativeTabs.creativeTabArray;
            int n = CreativeTabs.creativeTabArray.length;
            for (int k = 0; k < n; ++k) {
                CreativeTabs creativetabs = arrcreativeTabs[k];
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for (int i2 = 0; i2 < n; ++i2) {
            CreativeTabs creativetabs = arrcreativeTabs[i2];
    ...
    protected void renderToolTip(ItemStack stack, int x, int y) {
        if (selectedTabIndex == CreativeTabs.tabAllSearch.getTabIndex()) {
            ...
            if (creativetabs == null && stack.getItem() == Items.enchanted_book && (map = EnchantmentHelper.getEnchantments(stack)).size() == 1) {
                ...
                int n = CreativeTabs.creativeTabArray.length;
                for (int i = 0; i < n; ++i) {
                    CreativeTabs creativetabs1 = arrcreativeTabs[i];
    ...
    protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
        ...
        GuiContainerCreative.drawTexturedModalRect(this.guiLeft, this.guiTop, 0, 0, this.xSize, this.ySize);
        ...
        if (creativetabs.shouldHidePlayerInventory()) {
            GuiContainerCreative.drawTexturedModalRect(i, j + (int)((float)(k - j - 17) * this.currentScroll), 232 + (this.needsScrollBars() ? 0 : 12), 0, 12, 15);
        }
    ...
    protected void func_147051_a(CreativeTabs p_147051_1_) {
        ...
        GlStateManager.disableLighting();
        GuiContainerCreative.drawTexturedModalRect(l, i1, j, k, 28, j1);
        zLevel = 100.0f;
